var File = require('./file.js'),
    ...
    DataChunk = require('./data_chunk.js'),
		TreeChunk = require('./tree_chunk.js');
...
var Processors = {
	"FORM": function(chunk_info, parentChunk) {
		...
		return {innerChunkBuffer: chunk_info.ckData.slice(4, chunk_info.ckSize-4), additionalData: {fileType: parentChunk.type}};
	},
	"CAT ": function(chunk_info, parentChunk) {
		contentsType = chunk_info.ckData.toString('ascii', 0, 4);
		return {innerChunkBuffer: chunk_info.ckData.slice(4, chunk_info.ckSize-4), additionalData: {contentsType: contentsType}};
	},
	"LIST": function(chunk_info, parentChunk) {
		contentsType = chunk_info.ckData.toString('ascii', 0, 4);
		return {innerChunkBuffer: chunk_info.ckData.slice(4, chunk_info.ckSize-4), additionalData: {contentsType: contentsType, properties: {}}};
	},
	"PROP": function(chunk_info, parentChunk) {
		formType = chunk_info.ckData.toString('ascii', 0, 4);
		if(parentChunk.properties) {
			parentChunk.properties[formType] = chunk_info.ckData.slice(4, chunk_info.ckSize-4);
		}
	...
	"CMAP": function(chunk_info, parentChunk) {
		...
		for(var i=0;i<colorData.colorCount;i++) {
...
var parseChunkList = function(parser, parentChunk, buffer) {
	...
	while(options.offset < buffer.length-8) {
		...
		if(processed.innerChunkBuffer) {
			chunk = new TreeChunk(chunk_info.ckID, chunk_info.ckSize);
