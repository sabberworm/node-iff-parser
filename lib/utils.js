...
exports.smus_write_midi = function(file, stream) {
	if(file.type !== 'SMUS') {
		throw "Only SMUS files supported, "+file.type+" given";
	}
	console.log(require('util').inspect(file.content, true, 50));
	var midimal = require('midimal');
	var midi = new midimal.MIDI(stream);
	var instruments = [];
	file.content.chunksById('INS1').forEach(function(instrument_chunk) {
		instruments.push(instrument_chunk.name);
	});
	file.content.chunksById('TRAK').forEach(function(track_chunk) {
		var track = midi.track();
		var instrument = null;
		var rest = 0;
		track_chunk.notes.forEach(function(note, track_index) {
			...
			duration = Math.round(duration*51200);
			if(note.rest) {
				rest = duration;
				return;
			}
			...
			if(instrument !== note.instrument) {
				...
				if(instruments[instrument]) {
					track.instrument(midimal.utils.instrument().from_name(instruments[instrument]));
				} else {
					track.instrument(midimal.utils.instrument().id(instrument));
				}
			}
			track.note(midimal.utils.note().key(note.tone).velocity(note.volume).force(true), duration, rest);
			rest = 0;
		});
	});
	midi.write();
